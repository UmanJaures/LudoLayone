...
export class OnlineLudo {
     ...
     * @param {string} playerRole - 'P1' ou 'P2'
     */
    ...
    handleGameWinner(data) {
        ...
        if (winType === 'abandon') {
            if (isCurrentPlayer) {
                message = `FÉLICITATIONS ! Vous avez gagné par abandon de votre adversaire ! 🏆\n\nVotre adversaire a quitté la partie.`;
            } else {
    ...
    resyncWithServer(gameState) {
        ...
        if (gameState.positions && gameState.positions.P1 && gameState.positions.P2) {
